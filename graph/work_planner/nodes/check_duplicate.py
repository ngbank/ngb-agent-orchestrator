...
def check_duplicate(state: WorkPlannerState) -> dict:
    ...
    for workflow in workflows:
        if workflow["status"].is_active():
            error = (
                f"Workflow already in progress for {ticket_key} "
                f"(ID: {workflow['id']})"
            )
            click.echo(f"❌ {error}", err=True)
            click.echo(
                "   Cannot start a new workflow while one is active.", err=True
            )
            click.echo(
                "   Wait for the current workflow to complete or fail.", err=True
            )
            return {"error": error}
    ...
    if completed_count > 0:
        click.echo(
            f"⚠️  Warning: {completed_count} completed workflow(s) exist for {ticket_key}"
        )
