...
**Amendment (Topic 15).** Under the synthesizer model, the curator's `merge` and `subsume` strategies are retained *only* for exact or near-literal duplicates (repeat mining runs on the same workflow). Semantic consolidation of paraphrase variants — the very case the tradeoff paragraph above warned about — is no longer attempted at write time. Paraphrase variants co-exist in staging as separate rows, each carrying its own `provenance` chain; the synthesizer collapses them contextually when it renders the injection block for a specific ticket. The similarity threshold stays conservative for the same reason: false merges are more expensive than duplicated storage. The Curator trim completes this: the `occurrence_count` column was dropped (merges no longer bump a counter), confidence is stable-after-creation modulo human review and time-based decay, and contradictions populate a `conflicts_with` list rather than a blocking status.
