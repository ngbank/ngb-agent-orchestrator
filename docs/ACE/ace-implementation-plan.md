...
| 4.3 | feat | Injection-time synthesizer (AOS-274) | `ace/retrieval/synthesizer.py` renders retrieved items into a structured markdown document (development_rules / architectural_approach / testing_approach / known_pitfalls) via LLM. Prompt at `ace/retrieval/prompts/synthesize.md`. Durable content-addressed store `synthesized_context_blocks` keyed by `hash((ticket_key, applicability_filter, corpus_snapshot_id, recipe_target))`; a matching key skips the LLM call while preserving provenance joins from `ace_injection_events.block_cache_key`. See [15-ace-injection-synthesizer.md](15-ace-injection-synthesizer.md) |
