...
The proxy config generated by `orchestrator.utils._litellm_config_yaml` pins `litellm_settings.request_timeout: 240` (4 minutes). This bounds any single LLM call — healthy calls complete in seconds, but a runaway upstream stream (e.g. a model spinning on `reasoning_content` tokens during the plan recipe) is aborted after 4 minutes with an error, well under the plan-phase 5-minute ceiling.

The standalone dev proxy launched via [`bin/litellm-dev`](../bin/litellm-dev) uses the same generator, so the two paths cannot drift.
