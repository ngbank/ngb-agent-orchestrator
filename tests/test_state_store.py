...
def test_log_path_uses_xdg_state_home_by_default(test_db, monkeypatch):
    ...
    monkeypatch.setenv("XDG_STATE_HOME", "/tmp/aos-119-xdg")
    workflow_id = state_store.create_workflow(ticket_key="AOS-119")

    lp = log_path(workflow_id, "execute", ticket_key="AOS-119")

    expected_prefix = Path("/tmp/aos-119-xdg") / "ngb-agent-orchestrator" / "logs" / workflow_id
    assert str(lp).startswith(str(expected_prefix))


def test_log_path_honors_logs_dir_override(test_db, monkeypatch):
    """Explicit LOGS_DIR continues to override XDG-derived defaults."""
    from graph.utils import log_path

    monkeypatch.setenv("XDG_STATE_HOME", "/tmp/aos-119-xdg")
    monkeypatch.setenv("LOGS_DIR", "/tmp/aos-119-override")
    workflow_id = state_store.create_workflow(ticket_key="AOS-119")

    lp = log_path(workflow_id, "execute", ticket_key="AOS-119")

    expected_prefix = Path("/tmp/aos-119-override") / workflow_id
    assert str(lp).startswith(str(expected_prefix))
