"""Tests for the executor subgraph nodes (replaces the old monolithic execute_plan tests)."""
...
_PATCH_GOOSE_SESSION = "graph.executor.nodes.run_goose.goose_session"
...
def test_clone_repo_uses_mkdtemp_for_working_dir():
    ...
    with (
        patch(
            "graph.executor.nodes.clone_repo.tempfile.mkdtemp",
            return_value="/tmp/test-dir",
        ) as mock_mkdtemp,
        patch("graph.executor.nodes.clone_repo.run_and_tee") as mock_run,
        patch(
            "graph.executor.nodes.clone_repo.log_path",
...
def test_clone_repo_no_hardcoded_tmp_path():
    ...
    import graph.executor.nodes.clone_repo as module
...
def test_clone_repo_returns_workspace_paths_even_on_failure():
    ...
    with (
        ...
        patch("builtins.open", MagicMock()),
        patch(
            "graph.executor.nodes.clone_repo.tempfile.mkstemp",
            side_effect=[
                (0, "/tmp/summary.json"),
                (0, "/tmp/reasoning.txt"),
            ],
        ...
        patch("os.close"),
        patch(
            "graph.executor.nodes.clone_repo.tempfile.NamedTemporaryFile",
        ) as mock_ntf,
    ):
...
def test_run_goose_passes_existing_branch_and_comments():
    ...
    with (
        patch("graph.executor.nodes.run_goose.run_and_tee") as mock_run,
        patch("builtins.open", MagicMock()),
...
def test_persist_results_transitions_to_pending_pr_approval_on_success():
    ...
    with (
        patch("graph.executor.nodes.persist_results.update_status") as mock_update_status,
        patch("graph.executor.nodes.persist_results.update_execution_summary"),
        patch("graph.executor.nodes.persist_results.update_usage_summary"),
        patch(
            "graph.executor.nodes.persist_results.aggregate_token_usage",
            return_value={},
        ),
    ):
